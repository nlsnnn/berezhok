...
const getOrderProjection = `-- name: GetOrderProjection :one
...
type GetOrderProjectionRow struct {
...
func (q *Queries) GetOrderProjection(ctx context.Context, id uuid.UUID) (GetOrderProjectionRow, error) {
	row := q.db.QueryRow(ctx, getOrderProjection, id)
	var i GetOrderProjectionRow
	err := row.Scan(
		&i.OrderID,
		&i.CustomerID,
		&i.PartnerID,
